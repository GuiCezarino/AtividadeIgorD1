...
public class NewsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Noticia noticia : listaNoticia) {
			...
			response.getWriter().println("<h1>" + noticia.getTitulo() + "</h1>" +"<h2>" + noticia.getDescricao() + "</h2>" + "<p>" + noticia.getTexto() + "</p>");
			...
			response.getWriter().println("<hr/>"+"<h4> Nomee: " + comentario.getNome() + "</h4>" + "<p> Comentário: " + comentario.getComentario() + "</p>"+"<hr/>");
			...
			response.getWriter().println("<form method=\"post\" action=\"cadastraComentario.do\">"
									    ...
									    +"<input type=\"submit\" value=\"Comentar\" />" + "</form>"+"<hr/>");
		}
